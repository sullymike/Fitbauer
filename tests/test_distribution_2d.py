...
def test_bhf_quad_kernel_shape_and_order():
    ...
    k = build_bhf_quad_distribution_kernel(v, b, q, delta=0.0, gamma=0.16)
    assert k.shape == (v.size, b.size * q.size)
    # Orden C: primer índice BHF, segundo ΔEQ.
    expected = sextet_absorption(v, delta=0.0, quad=q[1], bhf=b[0], gamma=0.16)
...
def test_fit_bhf_quad_distribution_recovers_synthetic_peak():
    ...
    y = baseline - depth * sextet_absorption(
        v, delta=0.05, quad=true_q, bhf=true_b, gamma=0.16,
        int1=3.0, int2_rel=1.0, int3_rel=1.0,
    )
    res = fit_bhf_quad_distribution(
        v, y,
        delta=0.05, gamma=0.16,
...
def test_fit_bhf_quad_distribution_accepts_fixed_sharp_component():
    ...
    baseline = 1.0
    sharp_depth = 0.005
    dist_y = baseline - 0.014 * sextet_absorption(v, delta=0.0, quad=0.2, bhf=32.0, gamma=0.16)
    sharp_abs = sharp_depth * sextet_absorption(v, delta=0.0, quad=0.0, bhf=34.0, gamma=0.16)
    y = dist_y - sharp_abs
    res = fit_bhf_quad_distribution(
        v, y, delta=0.0, gamma=0.16,
        bmin=28.0, bmax=36.0, nbins_bhf=9,
        qmin=-0.4, qmax=0.8, nbins_quad=7,
