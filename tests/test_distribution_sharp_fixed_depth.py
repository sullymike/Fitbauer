...
def test_backend_splits_fixed_and_free_sharp_depths():
    ...
    components = [
        {"kind": "Singlete", "delta": -0.2, "gamma": 0.30, "depth": 0.012, "depth_fixed": True},
        {"kind": "Singlete", "delta": +0.4, "gamma": 0.40, "depth": 0.034, "depth_fixed": False},
    ]
    ...
    expected_fixed = 0.012 * singlet_absorption(v, delta=-0.2, gamma=0.30)
...
def test_fixed_sharp_depth_is_not_refitted_in_backend():
    ...
    y = 1.0 - 0.03 * singlet_absorption(v, delta=0.0, gamma=0.4, int1=1.0)
    centers = np.array([30.0, 40.0])
    weights = np.array([1.0, 1.0])
    sharp = [{"kind": "Singlete", "delta": 0.0, "gamma": 0.4, "int1": 1.0, "depth": 0.01, "depth_fixed": True}]
...
def test_free_sharp_depth_is_still_refitted_in_backend():
    ...
    centers = np.array([30.0, 40.0])
    weights = np.array([1.0, 1.0])
    sharp = [{"kind": "Singlete", "delta": 0.0, "gamma": 0.4, "int1": 1.0, "depth": 0.01, "depth_fixed": False}]
