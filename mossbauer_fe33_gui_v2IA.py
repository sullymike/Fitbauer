...
def fe57_sextet_positions(bhf_t: float = BHF_DEFAULT_T) -> np.ndarray:
    """Posiciones de las 6 líneas del sextete de Fe-57.

    Posiciones PUBLICADAS de α-Fe (patrón de velocidad estándar). Definen la
    calibración: un α-Fe ajusta a BHF = 33.0 T, igual que NORMOS. No sustituir
    por valores teóricos desde los momentos nucleares: darían el desdoblamiento
    ~0,4 % menor y el BHF saldría ~0,1 T demasiado alto.
    """
...
def read_ws5_counts(path: Path) -> np.ndarray:
    from core.folding import read_ws5_counts as _impl
    return _impl(path)


def _number_re() -> str:
    from core.folding import _number_re as _impl
    return _impl()


def read_normos_folding_point(path: Path) -> float | None:
    from core.folding import read_normos_folding_point as _impl
    return _impl(path)


def read_normos_plt_velocity(path: Path) -> float | None:
    from core.folding import read_normos_plt_velocity as _impl
    return _impl(path)


def read_normos_sidecar_params(path: Path) -> dict[str, float]:
    from core.folding import read_normos_sidecar_params as _impl
    return _impl(path)


def interp_channel_1based(counts: np.ndarray, channel: float) -> float:
    from core.folding import interp_channel_1based as _impl
    return _impl(counts, channel)


def fold_integer_or_half(counts: np.ndarray, center: float) -> tuple[np.ndarray, list[tuple[int, int]]]:
    from core.folding import fold_integer_or_half as _impl
    return _impl(counts, center)


def chi2_for_center(counts: np.ndarray, center: float) -> tuple[float, int]:
    from core.folding import chi2_for_center as _impl
    return _impl(counts, center)


def find_best_integer_or_half_center(counts: np.ndarray, cmin: float = 250.5, cmax: float = 262.5) -> float:
    from core.folding import find_best_integer_or_half_center as _impl
