...
\begin{itemize}[nosep]
  ...
  \item \textbf{Transmission integral} (v4.18): the exact model
        $T = \text{base} - L_{\text{source}} \otimes \bigl(1 - e^{-\tau(v)}\bigr)$,
        where $\tau$ is the \emph{optical} depth (the role the depth of each
        component takes on) and $L_{\text{source}}$ is the source line, of width
        \param{Source $\Gamma$} (fixed by default at 0.097~mm/s, the natural
        width; untick ``Fixed'' to refine it). Recommended for thick absorbers
        (effective thickness $t_a \gtrsim 1$): in the validation against NORMOS
        it recovers $\Gamma$ correctly up to $t_a = 30$, where the exponential
        saturation already biases.
\end{itemize}

\begin{truco}
For dense samples or very deep lines ($>15$--$20\%$), use \textbf{Thick} or the
\textbf{transmission integral}: with the thin model the relative areas (and
therefore the phase proportions) and the linewidths would come out biased.
\end{truco}

The calibration panel also includes an optional \param{Baseline curvature}
($v^2$ term, fixed at 0 by default): it absorbs the residual parabolic
geometric effect that survives folding. If the residuals show a smooth
curvature across the whole spectrum, free it instead of letting $\Gamma$ and
the depths compensate for it. Since v4.19 there are also \param{Baseline v$^3$} and
\param{Baseline v$^4$} terms (fixed at 0 by default): they complete parity
with NORMOS's polynomial background (BKG(4)/BKG(5) coefficients) for
high-order instrumental baselines.
...
The treatment of the \textbf{quadrupole} in the sextet is chosen by
right-clicking the $\Delta E_Q$ control of the component:

\begin{itemize}[nosep]
  \item \textbf{1st order} (perturbative): the classic rigid pattern.
  \item \textbf{Kündig fixed / powder}: positions by diagonalisation of the
        magnetic+quadrupolar Hamiltonian with axial EFG and angle $\beta$
        (appendix~\ref{ap:mates}).
  \item \textbf{Full Hamiltonian} (v4.18): general diagonalisation with a
        \emph{non-axial} EFG —component parameters $\eta$ (asymmetry) and
        $\varphi$— and with the \textbf{intensities of the 8 transitions
        computed from the eigenvectors} (Clebsch--Gordan coefficients, powder
        average), including the $\Delta m_I = \pm 2$ transitions that appear
        under strong mixing. It is the correct mode when the quadrupole
        interaction is comparable to the magnetic one or the EFG is not axial;
        it was validated against NORMOS-SITE and is exact (rotationally
        invariant) even where that code approximates.
  \item \textbf{Single-crystal Hamiltonian} (v4.19): like the previous one,
        but without averaging the $\gamma$ beam: its direction is fixed by
        the component angles $\theta_\gamma$ and $\varphi_\gamma$ (polar
        and azimuthal in the EFG principal-axes frame), with the
        \emph{coherent} sum over radiation channels. This is the mode for
        single crystals or oriented samples: in the axial limit with
        $\vec B \parallel z$ it reproduces the classic
        $3\!:\!0\!:\!1$ (beam $\parallel \vec B$) and $3\!:\!4\!:\!1$
        (beam $\perp \vec B$) patterns, and its isotropic average recovers
        the powder mode exactly. It is the counterpart of NORMOS-SITE's
        IFSC/BEX/GAX (note: NORMOS's GAX is offset by $90^\circ$ from
        $\varphi_\gamma$, as measured in the validation bench).
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Automatic global escalation} (v4.18, on by default): if the
        reduced $\chi^2$ is still high after the multistart, a differential
        evolution (DE) pass plus a local refinement is launched automatically.
        In the validation bank it completely removed the convergence failures
        in overlapped multi-site fits (from $\sim$40\,\% to 0 with $\pm 15\,\%$
        perturbed starts).
  \item \textbf{Channel integration} (v4.18): with wide channels (channel width
        $\gtrsim \Gamma/3$) the classic evaluation at the channel centre
        overestimates $\Gamma$; a value $>1$ integrates the model over the
        channel width with a Gauss--Legendre quadrature of that order.
  \item \textbf{Free lines} (v4.18): widens the $\delta$ limits to
        $\pm(v_{\max}+2)$~mm/s so that singlets can be used as free Lorentzian
        lines at any position (outside the physical $\delta$ range of
...
\begin{nota}
Since v4.18 up to \textbf{10 components} can be enabled (previously 6); the
validation bank demonstrated correct recovery of 10 simultaneous sites.
\end{nota}
...
\section{Which parameters you see}
\label{sec:params-visibles}

Each component panel shows \textbf{only} the parameters that can be adjusted
with the active type, intensity mode and quadrupole treatment. A doublet shows
neither $\BHF$ nor $\Gamma_3$; a first-order sextet shows no $\eta$, $\phi$,
$\beta$, \code{Bex}, \code{Gax} or texture, because those exist only with the
Hamiltonian treatment or in Texture mode.

Nothing is lost. They reappear the moment you change the treatment (right-click
on the sextet panel) or the intensity mode, and they keep whatever value they
held while hidden. Hiding them is presentation only: it changes neither the
model nor what enters the fit.

\begin{truco}
If you miss a parameter, check the quadrupole treatment and the intensity mode
first --- that is almost always the reason. These parameters used to be shown
greyed out; they cost three rows per component without being usable, and with
two or three components open they no longer fitted on screen.
\end{truco}

\paragraph{Compact parameters.} \menu{View > Compact parameters} removes the
slider from every parameter, so each takes a single row instead of two. A sextet
drops from 262 to 177 pixels tall and three stacked sextets from 786 to 531,
which is what lets you watch two or three components at once. It hides
\textbf{no} parameter: the numeric box still edits every one of them, with its
arrows and the mouse wheel; all you lose is the slider's coarse dragging. That
is the difference from a \emph{basic/advanced} mode --- no parameter can cease
to exist for you because you cannot see it. The setting also applies to the
calibration and distribution panels and is remembered between sessions.
