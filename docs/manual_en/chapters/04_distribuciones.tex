...
\subsection{Kernel physics: 1st order or Hamiltonian (v4.19)}
\label{sec:kernel-hc}

By default each subspectrum of the grid is the rigid 1st-order sextet. The
\menu{Kernel} selector of the panel switches it to the \textbf{powder
average of the full Hamiltonian}: each kernel column is computed by
diagonalising the magnetic+quadrupole Hamiltonian over all orientations of
the EFG with respect to $\vec B$ (with the $\eta$ control for non-axial
EFG). With this kernel, $\dEQ$ is no longer a rigid shift but the
\textbf{magnitude of the random EFG} —the exact analogue of the EXACT/QUP
mode of NORMOS-DIST, without truncating to perturbation theory—. Use it when
the quadrupole interaction is not small compared with the magnetic splitting
at the low end of the grid ($R = \dEQ/(0.323\,B) \gtrsim 0.2$): with the
1st-order kernel those spectra bias $\langle B\rangle$ and artificially
broaden $\sigma$. With $\dEQ = 0$ both kernels are identical. In powder
mode the kernel texture (area ratio of lines 2,5) is tied to the direction
of $\vec B$, as it should be.

\subsection{Polarized source (v4.20)}
\label{sec:fuente-polarizada}

For measurements with a \textbf{magnetized source} (emitting sextet),
enable \menu{Polarized source} in the panel: every absorber line is split
against every source line (36-line comb) with intensities given by
\emph{helicity selection} —in the aligned case (source and absorber along
the beam) only pairs with equal $\Delta m$ survive—. The controls are the
source field $B_{\rm src}$ and the source--beam and absorber--beam angles.
It is the counterpart of NORMOS-DIST's METHOD=4/POLAR mode, validated
against it (0.4\,\% of the peak).
...
\subsection{Anchoring the grid edges}
\label{sec:edge-anchor}

An extra weight can force $P \to 0$ at the first and last points of the grid.
It is the counterpart of NORMOS-DIST's \code{BETA1}/\code{BETA2}, which are added
to the corners of its smoothing matrix, and it is what the \code{BETA}/\code{LAMDA}
ratio of an imported \ruta{.JOB} becomes (\S\ref{sec:normos-job}).

Why it matters: without anchoring, if the true distribution extends beyond the
grid range, the Hesse--Rübartsch method \textbf{piles} all the excess weight onto
the extreme point. The result is a spurious peak that also drags the mean
$\langle \BHF \rangle$. With anchoring the fit cannot take refuge there and the
misfit becomes visible --- which is exactly what you want to see.

\begin{truco}
The \code{edge\_pileup} diagnostic warns when weight accumulates at an edge. If
it fires, the first move is to \textbf{widen the grid range}; anchoring is the
second line of defence, not a substitute for a grid that covers the physics.
\end{truco}
