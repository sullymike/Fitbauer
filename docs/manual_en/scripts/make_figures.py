...
def fig_load_alphafe() -> None:
    ...
    a1.axvline(center, color=MODEL_C, ls="--", lw=1.3,
               label=f"folding center ≈ {center:.1f}")
    a1.set_xlabel("Channel")
    a1.set_ylabel("Counts (norm.)")
    a1.set_title(f"Unfolded ({counts.size} channels)")
    a1.legend(loc="lower center", fontsize=8)
    # Dcha: espectro doblado en eje de velocidad.
    a2.plot(v, y, ".", color=DATA_C, ms=3.0)
    a2.set_xlabel("Velocity (mm/s)")
    a2.set_ylabel("Relative transmission")
    a2.set_title(f"Folded ({y.size} points)")
    fig.suptitle("α-Fe: from the unfolded to the folded spectrum", y=1.03)
...
def fig_sine_drive() -> None:
    ...
    a1.set_xlabel("Channel")
    a1.set_ylabel("Velocity (mm/s)")
    a1.set_title("Velocity law: $v_i = v_{max}\\,\\sin(2\\pi(i-c_0)/N)$",
                 fontsize=9)
    # Dcha: espectro SIN plegar (cada canal en su velocidad real, no monótono).
    a2.plot(v, T, ".", color=DATA_C, ms=2.5)
    a2.set_xlabel("Velocity (mm/s)")
    a2.set_ylabel("Relative transmission")
    a2.set_title("Unfolded spectrum (fitted as such)", fontsize=9)
    fig.suptitle("Sinusoidal drive: no folding; each channel at its real velocity",
                 y=1.03)
...
def fig_calibration_fit() -> None:
    ...
    ax.plot(v, y, ".", color=DATA_C, ms=3.2, label="Data")
    ax.plot(v, fit_curve, "-", color=MODEL_C, label="Fit (sextet)")
    ax.set_ylabel("Relative transmission")
    rchi = result["stats"].get("red_chi2", float("nan"))
    ax.set_title(f"α-Fe calibration · BHF = 33.0 T (fixed) · "
                 f"Fitted Vmax = {vmax_fit:.3f} mm/s · χ²ᵣ = {rchi:.3g}",
                 fontsize=9)
...
def fig_sim_components() -> None:
    ...
    for ax, (vx, kind, p, title) in zip(axs, [
            (v, "Singlete", p_sing, "Singlet"),
            (v, "Doblete", p_doub, "Doublet (ΔEQ = 0.80)"),
            (v6, "Sextete", p_sext, "Sextet (BHF = 33 T)")]):
        m = total_model(vx, 1.0, 0.0, [(kind, p)])
        ax.plot(vx, m, "-", color=MODEL_C)
        ax.set_title(title, fontsize=10)
        ax.set_xlabel("v (mm/s)")
    axs[0].set_ylabel("Transmission")
    fig.suptitle("Simulation of discrete components", y=1.02)
...
def fig_pbhf_distribution() -> None:
    ...
    axP.plot(centers, Pn_sharp, "-", color=DIST_C, label="Crystalline (sharp)")
    axP.plot(centers, Pn_broad, "--", color=DATA_C, label="Broad distribution")
    axP.set_xlabel("BHF (T)")
    axP.set_ylabel("P(BHF)")
    axP.set_title("Hyperfine field distribution")
    axP.legend(loc="upper left")

    axS.plot(v, sp_sharp, "-", color=DIST_C, label="Crystalline spectrum")
    axS.plot(v, sp_broad, "--", color=DATA_C, label="Distributed spectrum")
    axS.set_xlabel("Velocity (mm/s)")
    axS.set_ylabel("Transmission")
    axS.set_title("Simulated spectra")
