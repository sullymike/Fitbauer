...
mediante el diálogo \code{GlobalNeelFitDialog} (\code{gui/dialogs.py},
\textbf{Ajuste → Global Néel (T)}).
\end{abstract}
...
\begin{nota}[Alcance — v4.9.0+]
Los tres modelos descritos en este documento (\code{Relajacion}, \code{BlumeTjon}
y \code{NeelSize}) están completamente implementados en \code{core/physics.py}
e integrados en la GUI. El ajuste global multi-temperatura está disponible desde
\textbf{Ajuste → Global Néel (T)} (\code{GlobalNeelFitDialog} en
\code{gui/dialogs.py}). Para un único espectro conviene fijar parámetros con
información externa (TEM, magnetometría) o usar el ajuste global multi-temperatura
