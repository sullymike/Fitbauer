\noindent
Este documento describe los tres modelos de relajación magnética implementados
en Fitbauer, complementarios a los modelos preexistentes de singlete, doblete,
sextete y distribuciones Hesse--Rübartsch:
(1)~\code{Relajacion}: componente fenomenológica que interpola entre un sextete
bloqueado y un doblete superparamagnético aparente;
(2)~\code{BlumeTjon}: modelo dinámico de dos estados ($+\BHF \leftrightarrow -\BHF$)
con intercambio simétrico tipo Bloch--McConnell;
(3)~\code{NeelSize}: modelo Néel--Arrhenius con distribución lognormal de tamaños
de partícula, combinando el perfil \code{BlumeTjon} para cada diámetro.
El ajuste global simultáneo de espectros a varias temperaturas está disponible
desde \textbf{Ajuste → Global Néel (T)}.
...
\begin{figure}[htbp]\centering
  ...
  \caption{Modelo fenomenológico \code{Relajacion} \eqref{eq:rel-phen}. Al bajar
  la fracción bloqueada $f_b$ de $1$ (sexteto bloqueado) a $0$ (doblete
  superparamagnético aparente), la señal del sexteto se transfiere a un doblete
  central de área equivalente. No representa dos fases químicas, sino el efecto
  espectral de la relajación.}
...
La figura~\ref{fig:apx_relaxation} muestra la evolución continua entre estos
límites al aumentar la tasa de intercambio.

\begin{figure}[htbp]\centering
  \includegraphics[width=0.72\textwidth]{apx_relaxation}
  \caption{Colapso por relajación en el modelo de dos estados
  $+\BHF\leftrightarrow-\BHF$ \eqref{eq:exchange}. A tasa baja (abajo) el espectro
  es el \emph{sexteto estático}; al crecer la tasa las líneas se ensanchan y
  coalescen (\emph{régimen intermedio}); a tasa alta (arriba) las posiciones
  magnéticas se promedian a cero y queda una \emph{única línea colapsada}. Curvas
  desplazadas verticalmente para comparar.}
  \label{fig:apx_relaxation}
\end{figure}

%======================================================================
\subsection{Polarización de poblaciones}
\label{sec:relax-polarizacion}

Por defecto los dos estados son igual de probables. El parámetro
\param{polarización} $P \in [-1, 1]$ los desequilibra,
\begin{equation}
  p_\pm = \frac{1 \pm P}{2},
\end{equation}
lo que describe un campo externo, o una anisotropía, que favorece una de las dos
orientaciones del momento. Con $P = 0$ no cambia nada. En NORMOS es
$\code{SPN} = \BHF/\code{BSAT}$.

Úselo cuando un espectro de relajación salga asimétrico y la asimetría no se
explique ni por la anchura ni por el desplazamiento isomérico. La
implementación reproduce la rutina \code{ISIRLX} de NORMOS \emph{exactamente}
--- rms por debajo de $10^{-12}$, no de forma aproximada.
...
\begin{figure}[htbp]\centering
  ...
  \caption{Néel--Arrhenius: bloqueo por tamaño. \emph{Izquierda}: la tasa de
  relajación $\nu(d,T)$ cae al crecer el diámetro; las partículas pequeñas relajan
  rápido (superparamagnéticas, colapsan a singlete/doblete) y las grandes quedan
  \emph{bloqueadas} (sexteto). Una partícula se ve bloqueada cuando $\nu$ baja de
  la ventana de observación Mössbauer (banda gris); al bajar $T$ el bloqueo se
  desplaza a diámetros menores. \emph{Derecha}: sobre una distribución lognormal
  de tamaños, a $T$ fija el diámetro de bloqueo $d_B$ separa la fracción
  superparamagnética de la bloqueada, y el espectro es la mezcla de ambas.}
  \label{fig:apx_neel}
\end{figure}
...
Fitbauer puede ajustar simultáneamente varios espectros a distintas
temperaturas. Para cada espectro $r$ se tiene $T_r(v)$ y temperatura $T_r$. El modelo comparte los parámetros físicos
\begin{equation}
