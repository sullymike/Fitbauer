...
\subsection{Física del kernel: 1er orden o Hamiltoniano (v4.19)}
\label{sec:kernel-hc}

Por defecto cada subespectro de la malla es el sexteto rígido de 1er orden.
El selector \menu{Kernel} del panel permite cambiarlo por el \textbf{promedio
de polvo del Hamiltoniano completo}: cada columna del kernel se calcula
diagonalizando el hamiltoniano magnético+cuadrupolar sobre todas las
orientaciones del EFG respecto a $\vec B$ (con el control $\eta$ para EFG no
axial). Con este kernel, $\dEQ$ deja de ser un desplazamiento rígido y pasa a
ser el \textbf{módulo del EFG aleatorio} —el análogo exacto del modo
EXACT/QUP de NORMOS-DIST, sin truncar a teoría de perturbaciones—. Úselo
cuando la interacción cuadrupolar no sea pequeña frente al desdoblamiento
magnético en la parte baja de la malla ($R = \dEQ/(0{,}323\,B) \gtrsim
0{,}2$): con el kernel de 1er orden esos espectros sesgan $\langle B\rangle$
y ensanchan $\sigma$ artificialmente. Con $\dEQ = 0$ ambos kernels son
idénticos. En modo polvo la textura del kernel (relación de áreas de las
líneas 2,5) queda ligada a la dirección de $\vec B$, como corresponde.

\subsection{Fuente polarizada (v4.20)}
\label{sec:fuente-polarizada}

Para medidas con \textbf{fuente magnetizada} (sexteto emisor), active
\menu{Fuente polarizada} en el panel: cada línea del absorbente se desdobla
contra cada línea de la fuente (peine de 36 líneas) con las intensidades
dadas por la \emph{selección de helicidad} —en el caso alineado
(fuente y absorbente a lo largo del haz) solo sobreviven los pares con
$\Delta m$ igual—. Los controles son el campo de la fuente $B_{\rm src}$ y
los ángulos fuente--haz y absorbente--haz. Es el análogo del modo
METHOD=4/POLAR de NORMOS-DIST, validado contra él (0.4\% del pico).
...
\subsection{Anclaje de los bordes de la malla}
\label{sec:edge-anchor}

Un peso adicional puede forzar $P \to 0$ en el primer y el último punto de la
malla. Es el equivalente de los \code{BETA1}/\code{BETA2} de NORMOS-DIST, que
él suma a las esquinas de su matriz de suavizado, y es en lo que se convierte la
razón \code{BETA}/\code{LAMDA} de un \ruta{.JOB} importado
(\S\ref{sec:normos-job}).

Por qué importa: sin anclaje, si la distribución real se sale del rango de la
malla, el método de Hesse--Rübartsch \textbf{apila} todo el peso sobrante en el
punto extremo. Sale un pico espurio que además arrastra el valor medio
$\langle \BHF \rangle$. Con anclaje el ajuste no puede refugiarse ahí y el
desajuste se hace visible --- que es justo lo que se quiere ver.

\begin{truco}
El diagnóstico \code{edge\_pileup} avisa cuando se acumula peso en un borde. Si
salta, lo primero es \textbf{ampliar el rango de la malla}; el anclaje es la
segunda línea de defensa, no un sustituto de una malla que cubra la física.
\end{truco}
