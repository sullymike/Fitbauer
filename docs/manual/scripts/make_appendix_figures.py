...
def fig_lineshapes():
    ...
    for a in (ax, axlog):
        a.plot(v, _lorentz(v, 0, fwhm), color=C_LOR, label="Lorentziana")
        a.plot(v, _voigt(v, 0, 0.7 * fwhm, 0.30), color=C_VOI, label="Voigt")
        a.plot(v, _gauss(v, 0, fwhm), color=C_GAU, ls="--", label="Gaussiana")
        a.axhline(0.5, color=C_GRAY, lw=0.7, ls=":")
        a.set_xlabel("$v-v_0$ (unidades de FWHM)")
    ax.set_ylabel("perfil (altura 1)")
    ax.set_title("Perfiles a igual FWHM")
    ax.annotate("media altura", (3.4, 0.52), color=C_GRAY, fontsize=8)
    axlog.set_yscale("log")
    axlog.set_ylim(1e-3, 1.3)
    axlog.set_title("Colas (escala log)")
...
def fig_zeeman():
    ...
    axl.set_title("Desdoblamiento Zeeman del $^{57}$Fe y transiciones "
                  r"permitidas ($\Delta m=0,\pm1$)")
    ...
    axs.set_title("Sexteto resultante: 6 líneas, intensidades 3:2:1:1:2:3")
    axs.set_xlim(-8, 8)
...
def fig_regularization():
    ...
    titles = [r"$\alpha$ pequeño: sub-regularizado",
              r"$\alpha$ óptimo (esquina L)",
              r"$\alpha$ grande: sobre-regularizado"]
    data = [under, true, over]
    cols = [C_LOR, C_GAU, C_VOI]
    for a, d, t, c in zip(axes, data, titles, cols):
        a.fill_between(B, 0, true, color="k", alpha=0.08, label="verdadera")
        a.plot(B, d, color=c, lw=1.8)
        a.set_title(t, fontsize=9.5)
        a.set_xlabel("$B_{\\mathrm{hf}}$ (T)")
    axes[0].set_ylabel("$P(B_{\\mathrm{hf}})$")
    axes[0].annotate("oscilaciones\nespurias", (30, 0.55), color=C_LOR, fontsize=8)
    axes[2].annotate("estructura\nlavada", (30, 0.55), color=C_VOI, fontsize=8)
...
def fig_relaxation():
    v = np.linspace(-8, 8, 1600)
    rates = [(0.02, "lento: sexteto estático"),
             (0.6, "intermedio"),
             (2.5, "intermedio-rápido"),
             (30.0, "rápido: singlete colapsado")]
    ...
    ax.set_ylabel("transmisión (desplazada)")
    ax.set_title(r"Colapso por relajación al crecer la tasa de intercambio $\nu$")
    ax.set_yticks([])
    ax.set_xlim(-8, 8)
...
def fig_thickness():
    ...
    axm.plot(A, A, color=C_GRAY, ls="--", label=r"delgado ($C\to\infty$)")
    for C, c in [(1.0, C_VOI), (0.4, C_LOR)]:
        axm.plot(A, C * (1 - np.exp(-A / C)), color=c, label=f"$C={C:g}$")
        axm.axhline(C, color=c, lw=0.7, ls=":")
    axm.set_xlabel("$A_{\\mathrm{tot}}$ (absorción sin saturar)")
    axm.set_ylabel("$A_{\\mathrm{eff}}$ (efectiva)")
    axm.set_title("Saturación de la absorción")
    ...
    axs.plot(v, 1 - Atot, color=C_GRAY, ls="--", label="delgado $1-A$")
    for C, c in [(1.0, C_VOI), (0.4, C_LOR)]:
        axs.plot(v, 1 - C * (1 - np.exp(-Atot / C)), color=c, label=f"grueso $C={C:g}$")
    axs.set_xlabel("velocidad (mm/s)")
    axs.set_ylabel("transmisión")
    axs.set_title("Una línea: saturación y ensanchamiento")
    axs.set_ylim(-0.7, 1.06)
    axs.legend(loc="lower right", fontsize=8)
    axs.annotate("$1-A<0$:\nel modelo lineal\nse rompe", (-3.8, -0.42),
                 color=C_GRAY, fontsize=8, ha="left", va="center")
    axs.annotate("más plana\ny más ancha", (1.25, 0.55), color=C_LOR, fontsize=8)
...
def fig_texture():
    ...
    marks = [(0.0, "3:0:1\neje ∥ haz", (8, 4), "left"),
             (2 / 3, "3:2:1\npolvo aleatorio", (-6, 10), "center"),
             (1.0, "3:4:1\nlámina ⊥ haz", (-8, 2), "right")]
    for tv, lab, off, ha in marks:
        y = 4 * tv / (2 - tv)
        axr.plot(tv, y, "o", color=C_LOR, ms=6)
        axr.annotate(lab, (tv, y), textcoords="offset points", xytext=off,
                     fontsize=8, ha=ha)
    axr.set_xlabel(r"parámetro de textura $t=\sin^2\theta$")
    axr.set_ylabel(r"$i_2$ (intensidad de las líneas 2 y 5)")
    axr.set_title("Textura de un sexteto")
    ...
    for tv, c, lab in [(0.0, C_GAU, "$t=0$  (3:0:1)"),
        ...
        off += 0.75
    axs.set_xlabel("velocidad (mm/s)")
    axs.set_ylabel("transmisión (desplazada)")
    axs.set_title("Las líneas 2 y 5 crecen con $t$")
    axs.set_yticks([])
    axs.set_xlim(-8, 8)
...
def fig_losses():
    ...
    axl.set_xlabel("residuo normalizado $r$")
    axl.set_ylabel(r"pérdida $\frac{1}{2}\rho(r^2)$")
    axl.set_title("Función de pérdida")
    axl.set_ylim(0, 8)
    axl.legend(fontsize=8)
    for lab, (c, ls, y) in W.items():
        axw.plot(r, y, color=c, ls=ls, label=lab)
    axw.set_xlabel("residuo normalizado $r$")
    axw.set_ylabel("peso efectivo (IRLS)")
    axw.set_title("Peso de cada punto")
    axw.set_ylim(0, 1.08)
    axw.annotate("los atípicos\npesan menos", (3.2, 0.30), color=C_LOR, fontsize=8)
...
def fig_is():
    ...
    axp.set_title("Distribución de $\\delta$ (estados de oxidación)")
    axp.set_ylim(0, 1.25)
    ...
    axs.set_xlabel("velocidad (mm/s)")
    axs.set_ylabel("transmisión")
    axs.set_title("Espectro: dos entornos solapados")
...
def fig_2d_contour():
    ...
    axt.set_title("Distribución conjunta $P(B_{\\mathrm{hf}},\\Delta E_Q)$ y marginales")
...
def fig_neel():
    ...
    axl.text(2.4, 7.5, "ventana\nMössbauer", fontsize=8, color=C_GRAY, va="center")
    axl.set_xlabel("diámetro $d$ (nm)")
    axl.set_ylabel(r"$\log_{10}\nu$  (s$^{-1}$)")
    axl.set_title("Tasa de relajación de Néel")
    axl.set_ylim(0, 11)
    axl.legend(fontsize=8, loc="lower left")
    axl.annotate(r"SPM (rápido): $\nu$ alta", (6.2, 9.5), fontsize=8, color="#444")
    axl.annotate(r"bloqueado (lento): $\nu$ baja", (10.2, 1.0), fontsize=8, color="#444")
    ...
    axr.fill_between(dd, 0, f, where=dd < d_b, color=C_LOR, alpha=0.25, label="SPM (colapsado)")
    axr.fill_between(dd, 0, f, where=dd >= d_b, color=C_VOI, alpha=0.25, label="bloqueado (sexteto)")
    axr.axvline(d_b, color=C_GRAY, ls="--", lw=1.0)
    axr.text(d_b + 0.2, f.max() * 0.85, r"$d_B$", color=C_GRAY, fontsize=9)
    axr.set_xlabel("diámetro $d$ (nm)")
    axr.set_ylabel("$f(d)$ (lognormal)")
    axr.set_title("Reparto por tamaño a $T$ fija")
...
def fig_quadrupole():
    ...
    ax.set_xlabel("velocidad (mm/s)")
    ax.set_ylabel("transmisión (desplazada)")
    ax.set_title("Efecto del cuadrupolo sobre un sexteto (primer orden)")
    ax.set_yticks([])
    ax.set_xlim(-8, 8)
...
def fig_powder():
    ...
    singles = [(0.0, C_VOI, "monocristal  β = 0° (∥)"),
               (np.pi / 2, C_GAU, "monocristal  β = 90° (⊥)")]
    off = 0.0
    ...
    ax.text(8.7, 1 + off, "promedio de polvo", ha="right", va="bottom",
            color="k", fontsize=9)
    ax.set_xlabel("velocidad (mm/s)")
    ax.set_ylabel("transmisión (desplazada)")
    ax.set_title(r"Promedio policristalino sobre las orientaciones $\beta$"
                 " (con $\\Delta E_Q\\neq0$)")
    ax.set_yticks([])
    ax.set_xlim(-9, 9)
...
def fig_lsurface():
    ...
    ax.annotate("esquina elegida", (-2.2, -2.2), textcoords="offset points",
                xytext=(12, -2), color="white", fontsize=9, va="center")
    cb = fig.colorbar(cf, ax=ax)
    cb.set_label(r"RMS ($\times10^{-3}$)")
    ax.set_xlabel(r"$\log_{10}\alpha_{B}$ (campo)")
    ax.set_ylabel(r"$\log_{10}\alpha_{Q}$ (cuadrupolo)")
    ax.set_title("L-surface: RMS frente a las dos regularizaciones")
    ax.grid(False)
...
def fig_relax_phenom():
    ...
    fig, ax = plt.subplots(figsize=(7.6, 5.4))
    rows = [(1.0, C_VOI, "$f_b=1$  (bloqueado: sexteto)"),
            (0.66, "#7b3ba1", "$f_b=0{,}66$"),
            (0.33, "#c0561f", "$f_b=0{,}33$"),
            (0.0, C_LOR, "$f_b=0$  (SPM: doblete)")]
    off = 0.0
    ...
    ax.set_xlabel("velocidad (mm/s)")
    ax.set_ylabel("transmisión (desplazada)")
    ax.set_title(r"Modelo fenomenológico: interpolación sexteto$\leftrightarrow$doblete")
    ax.set_yticks([])
    ax.set_xlim(-9, 9)
...
def fig_profile():
    ...
    ax.plot(th, chi_cov, color=C_GRAY, ls="--", label=r"covarianza (parábola)")
    ax.plot(th, chi_true, color=C_VOI, lw=1.8, label="verosimilitud perfilada")
    ...
    ax.text(0.22, 0.28, "IC asimétrico\n(realista)", color=C_VOI, fontsize=8, ha="center")
    ax.set_xlabel(r"parámetro $\theta$ (desde el óptimo, en $\sigma$)")
    ax.set_ylabel(r"$\chi^2-\chi^2_{\min}$")
    ax.set_title("Barras de error: perfil de verosimilitud vs covarianza")
    ax.set_ylim(0, 4)
...
def fig_multistart():
    ...
    for xs, c in starts:
        ax.plot(xs, 1.15, "o", color=c, ms=7,
                label=("arranques" if not labeled else None))
        labeled = True
    ...
    ax.plot(1.2, costf(1.2), "*", ms=20, color="#f97316", mec="k", mew=0.7,
            label="mínimo global")
    ax.text(-2.3, costf(-2.3) - 0.12, "mínimo\nlocal", ha="center", fontsize=8, color="#555")
    ax.set_xlabel("parámetro")
    ax.set_ylabel("coste $\\mathcal{C}$")
    ax.set_title("Arranque múltiple: escapar de mínimos locales")
...
def fig_vbf():
    ...
    comps = [(46.0, 2.2, 1.00, "sitio 1"), (50.0, 1.4, 0.55, "sitio 2"),
             (33.0, 4.0, 0.30, "cola de desorden")]
    ...
    ax.plot(B, total, color="k", lw=1.9, label=r"$P(B_{\mathrm{hf}})$ (suma VBF)")
    ax.set_xlabel(r"$B_{\mathrm{hf}}$ (T)")
    ax.set_ylabel(r"$P(B_{\mathrm{hf}})$")
    ax.set_title("VBF: la distribución como suma de gaussianas")
    ax.set_ylim(0, 1.3)
...
def main() -> int:
    ...
    fig_multistart()
    fig_vbf()
    print("Figuras de anexo generadas en docs/manual/img/.")
    return 0
