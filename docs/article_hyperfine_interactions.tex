...
The program implements discrete fitting of up to three simultaneous components
(singlets, doublets, and sextets), hyperfine field distributions $P(B_\mathrm{hf})$
and quadrupole-splitting distributions $P(\Delta E_Q)$ with Tikhonov regularisation
(Hesse--R\"{u}bartsch method), and relaxation models including an empirical
blocked-fraction approach and a two-state Blume--Tjon treatment.
...
and freely available under an open licence.
...
formally validated uncertainty-quantification framework; (iii) modern,
cross-platform graphical interface in three languages.
...
A thick-absorber correction $T(v) = B + sv - C[1-\exp(-A_\mathrm{tot}/C)]$,
where $C$ is the saturation scale parameter, is available as an option.
...
$\sigma$) is optionally available.

\subsection{Discrete components}
\label{sec:discrete}

Up to six spectral components can be superimposed; each is one of five
types: singlet, doublet, sextet, and two relaxation types
(Section~\ref{sec:relax}).

\subsubsection{Line profile}

The default Lorentzian profile has half-width at half-maximum (HWHM) $\gamma$:
\begin{equation}
  L(v;\,c,\gamma) = \frac{\gamma^2}{(v-c)^2 + \gamma^2}.
\end{equation}
A Voigt profile (convolution with a Gaussian of width $\sigma$) is also
available, evaluated via the Faddeeva function
(\texttt{scipy.special.wofz}), and normalised to its peak independently
of velocity sampling.
...
Three relaxation component types can be combined freely with discrete phases.

\textit{Phenomenological (Relajacion).}
Interpolates linearly between a blocked sextet and a superparamagnetic
doublet via an effective blocked fraction $f_\mathrm{eff}$ that depends on
the blocked-fraction parameter $f_b$ and the relaxation rate
$\log_{10}\nu$.

\textit{Two-state dynamic model (BlumeTjon).}
Implements the stochastic Liouville equation for the
$+B_\mathrm{hf} \leftrightarrow -B_\mathrm{hf}$ process following
Blume and Tjon~\cite{BluemeTjon1968}. A single parameter
$\log_{10}\nu$ drives the lineshape from slow (sextet) to fast
(singlet/doublet) limits through the coalescence regime.

\textit{N\'{e}el--Arrhenius with size distribution (NeelSize).}
Models a polydisperse nanoparticle assembly via a lognormal diameter
distribution $p(d;\,d_{50},\sigma_\ell)$. Each size bin is converted
to a relaxation rate via the N\'{e}el--Arrhenius
equation~\cite{Neel1949,Brown1963}:
\begin{equation}
  \nu(d,T) = \frac{1}{\tau_0}
             \exp\!\left(-\frac{K_\mathrm{eff}\,V(d)}{k_\mathrm{B}\,T}\right),
\end{equation}
and the total spectrum is a weighted sum of BlumeTjon spectra over size
bins. A multi-temperature global-fit backend shares physical parameters
across spectra.
...
and regularised (Tikhonov second-difference or Total Variation) by
minimising
\begin{equation}
...
a compromise value, and Generalised Cross-Validation (GCV).
...
$P(B_\mathrm{hf},\Delta E_Q)$, $P(\delta,\Delta E_Q)$ and
$P(B_\mathrm{hf},\delta)$ are also available, with independent
regularisation parameters in each direction:
\begin{equation}
  \|\mathbf{y}-\mathbf{X}\mathbf{p}\|_w^2
  + \alpha_B\|\mathbf{D}^2_B\mathbf{p}\|^2
  + \alpha_Q\|\mathbf{D}^2_Q\mathbf{p}\|^2, \quad \mathbf{p}\geq 0.
\end{equation}
...
\begin{description}
  ...
  \item[\texttt{core/fit\_engine.py}] Least-squares optimisation
    via \texttt{scipy.optimize.least\_squares} (Levenberg--Marquardt
    by default), multi-start initialisation, parameter bounds,
    covariance-matrix uncertainties, bootstrap resampling,
    and profile-likelihood intervals.

  \item[\texttt{core/folding.py}] Spectrum loading (WS5 XML and
    single-column ASCII ADT formats), automatic folding-point search
    by chi-squared minimisation, and Doppler-corrected velocity axis.
...
\begin{figure*}[t]
  ...
  \caption{Fitbauer main window showing a loaded M\"{o}ssbauer spectrum
           (folded, $^{57}$Co/Rh source, $\alpha$-Fe absorber).
           The left panel displays status information and calibration parameters;
           the right panel shows the interactive Plotly canvas.}
  \label{fig:gui-main}
\end{figure*}
...
\begin{figure*}[t]
  ...
  \caption{Discrete fitting of a two-component spectrum (magnetite Fe$_3$O$_4$).
           Individual subspectra are shown in colour (site A in blue, site B in red);
           the total model in black; residuals in the lower panel.}
  \label{fig:gui-fit}
\end{figure*}
...
\emph{Discrete mode} supports up to three simultaneous components
of any type (singlet, doublet, sextet), with independent free/fixed
flags for each parameter, linear equality and inequality constraints,
and multi-start global optimisation (up to ten random restarts from
Latin-hypercube-sampled initial conditions).
The noise model can be Gaussian (covariance-matrix weights) or
Poissonian (per-channel weights proportional to $1/c_i$).

\emph{Distribution mode} (Fig.~\ref{fig:gui-dist}) computes
$P(B_\mathrm{hf})$ or $P(\Delta E_Q)$ over a user-defined grid
with up to four simultaneously fitted sharp components.
...
\begin{figure}[t]
  ...
  \caption{Distribution fitting of a disordered iron oxide.
           The inset shows the reconstructed $P(B_\mathrm{hf})$
           distribution as a function of the hyperfine field.}
  \label{fig:gui-dist}
\end{figure}
...
\begin{figure}[t]
  ...
  \caption{L-curve tool for automatic regularisation parameter selection.
           Left: $\|\mathbf{r}\|$ vs.\ $\|\mathbf{L}\cdot\mathbf{P}\|$;
           right: RMS residual and $\tilde{\chi}^2$ as a function
           of $\alpha$.}
...
A \emph{batch fitting} mode applies a starting template sequentially
to an arbitrary list of spectra, using warm-start from the previous
fit. An interactive \emph{minimum-detection} tool identifies multiple
local minima in the $\chi^2$ surface.
\emph{Reports} are exportable to PDF/Markdown.
A \emph{web API} mode serves fit results as JSON for integration
with laboratory information management systems.
The interface is fully trilingual (Spanish, English, French)
via a \texttt{gettext}-compatible i18n layer.

% ─────────────────────────────────────────────────────────────────────────────
\section{Validation suite}
\label{sec:validation}

Version 4.11.0 includes \texttt{tests/test\_synthetic\_validation.py},
...
All spectra consist of 512 channels with a sinusoidal velocity drive;
...
Python packages (NumPy, SciPy, PySide6, Plotly), the exact
...
Real-time visual feedback during fitting, interactive Plotly
figures with hover information, batch processing of series of
spectra, and a clean Python API for integration into automated
workflows are features not available in legacy programs.
...
distinguishing characteristics: complete separation of physics and GUI
enabling headless/CLI use; three independent uncertainty-estimation
methods (covariance matrix, bootstrap, profile likelihood); a
comprehensive automated validation suite including Monte Carlo pull
statistics; built-in phase identification and batch processing;
...
common iron-bearing minerals, calibration templates,
and documentation in Spanish, English, and French.
