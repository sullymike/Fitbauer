...
\paragraph{$\alpha_B$, $\alpha_Q$ adimensionales.} Igual que en 1D, los $\alpha$
que fija el usuario no son los pesos brutos: cada eje se \textbf{adimensionaliza}
de forma independiente,
$\alpha_{B,\text{eff}}=\alpha_B\,\lambda_B\,c_0$ y
$\alpha_{Q,\text{eff}}=\alpha_Q\,\lambda_Q\,c_0$, con
$\lambda=\lVert A_{\text{dist}}\rVert_F^2/\lVert L_{\text{eje}}\rVert_F^2$
(norma de Frobenius de los operadores blanqueados) y $c_0=10^{-4}$
(\texttt{\small ALPHA\_REF\_SCALE}). Así $\log_{10}\alpha_B\approx\log_{10}\alpha_Q\approx0$
es el equilibrio natural en cada eje, independiente de $N$, ruido y profundidad.
El sistema aumentado usa $\sqrt{\alpha_{\cdot,\text{eff}}}\,L_\cdot$. En la implementación inicial se usa una malla rectangular y los
...
\begin{figure}[htbp]\centering
  ...
  \caption{Distribución conjunta $P(B_{\mathrm{hf}},\Delta E_Q)$ y sus
  marginales. El mapa central muestra dos poblaciones: una nube \emph{inclinada}
  (campo alto, cuadrupolo pequeño), cuya inclinación indica correlación aparente
  $\rho_{BQ}$, y un segundo sitio aislado. Arriba y a la derecha, las marginales
  $P_B$ y $P_Q$ obtenidas integrando el mapa; por sí solas no revelan la
  correlación, que sólo se ve en el mapa 2D.}
...
\begin{figure}[htbp]\centering
  ...
  \caption{L-surface de regularización 2D, análoga a la L-curve pero con dos
  parámetros. El RMS es bajo cuando ambas regularizaciones son débiles (esquina
  inferior izquierda: el mapa ajusta el ruido) y crece al reforzarlas (arriba a
  la derecha: sobre-suavizado). La esquina de compromiso (estrella) equilibra
  ajuste y suavidad en cada eje; conviene comprobar que la solución es estable
  alrededor de ella.}
