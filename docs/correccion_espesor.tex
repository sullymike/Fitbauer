...
\begin{figure}[htbp]\centering
  ...
  \caption{Absorbente delgado frente a grueso. \emph{Izquierda}: la absorción
  efectiva $A_{\text{eff}}=C(1-e^{-A_{\text{tot}}/C})$ satura acotada por $C$,
  mientras el modelo delgado ($C\to\infty$) crece sin límite. \emph{Derecha}:
  sobre una línea profunda, el modelo delgado $1-A$ llegaría a valores negativos
  imposibles; la corrección de espesor da líneas \emph{menos profundas y más
  anchas}, que es el efecto físico de la saturación.}
...
\begin{itemize}[leftmargin=*]
  ...
  \item En modo distribución, $P(\BHF)$ se muestra ya des-saturada; su área
        integrada refleja la fracción real.
\end{itemize}
